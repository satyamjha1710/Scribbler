...
<?php
	if(isset($_POST['mdelete']))
	{
		...
		mysqli_query($link,"delete from users where user_id=$dm_id");
	}
	if(isset($_POST['fdelete']))
	{
		$df_id=intval($_POST['user_female_id']);
		mysqli_query($link,"delete from users where user_id=$df_id");
	}
?>
<?php
...
<body>
<?php
	$all_users_que=mysqli_query($link,"select * from users");
...
<?php
	$all_male_que=mysqli_query($link,"select * from users where Gender='Male' order by user_id desc");
...
?>
    	<tr>
        	...
        	<td> <a href="../Scribbler_view_profile/view_profile.php?id=<?php echo $male_info[0]; ?>" style="text-transform:capitalize; color:#3B59A4; text-decoration:none;" onMouseOver="user_name_underLine(<?php echo $male_info[0]; ?>)" onMouseOut="user_name_NounderLine(<?php echo $male_info[0]; ?>)" id="uname<?php echo $male_info[0]; ?>"> <?php echo $male_info[1]; ?> </a> </td>
...
<?php
	$all_female_que=mysqli_query($link,"select * from users where Gender='Female' order by user_id desc");
...
?>
    	<tr>
        	...
        	<td> <a href="../Scribbler_view_profile/view_profile.php?id=<?php echo $female_info[0]; ?>" style="text-transform:capitalize; color:#3B59A4; text-decoration:none;" onMouseOver="user_name_underLine(<?php echo $female_info[0]; ?>)" onMouseOut="user_name_NounderLine(<?php echo $female_info[0]; ?>)" id="uname<?php echo $female_info[0]; ?>"> <?php echo $female_info[1]; ?> </a> </td>
